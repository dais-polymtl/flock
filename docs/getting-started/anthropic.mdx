...
CREATE MODEL(
   ...
   'claude-sonnet-4-5',
   'anthropic',
   {"tuple_format": "json", "max_batch_size": 32, "model_parameters": {"temperature": 0.7, "max_tokens": 1024}}
);
```
...
Anthropic has rate limits based on your plan tier. Monitor your usage in the [Anthropic Console](https://console.anthropic.com/). For high-volume workloads, use appropriate `max_batch_size` settings and consider setting a per-model `rate_limit` in Flock to throttle requests per minute (see [Models Management](/resource-management/models)).
