...
Default: `16`. This default may change in a future release, so set `max_batch_size` explicitly if you rely on it. Some models answer less accurately when many tuples share one request; if answers look mixed up between rows, lower it.
...
`threshold` applies to `typesafe` models, which answer `llm_filter` with a probability rather than a yes or no. A probability at or above the threshold counts as `true`. It must be a number between 0 and 1.

There is no default: `llm_filter` on a `typesafe` model fails until a threshold is set here or inline on the model.

```sql
CREATE MODEL('strict-jev', 'jev-latest', 'typesafe', {"threshold": 0.8});
```

It can also be set inline on the model, which overrides the saved one: `{'model_name': 'strict-jev', 'threshold': 0.9}`. Other providers ignore it.
...
```sql
-- User-defined model (only tuple_format, max_batch_size, batch_size, model_parameters, is_async, rate_limit, usage_limit, and threshold allowed in JSON)
