...
There is no default: `llm_filter` on a `typesafe` model is refused until a threshold is set here, inline, or per call.

```sql
CREATE MODEL('strict-jev', 'jev-latest', 'typesafe', {"threshold": 0.8});
```

It can also be set inline on the model, or per call in `llm_filter`'s prompt struct, which overrides the model's (see [`llm_filter`](/scalar-functions/llm-filter)). Other providers ignore it.
