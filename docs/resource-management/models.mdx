...
Default: `16`, or `128` for `typesafe` models, where larger batches are cheaper per row.
...
It can also be set inline on the model, or per call in `llm_filter`'s prompt struct, which overrides the model's (see [`llm_filter`](/scalar-functions/llm-filter)). Other providers return no probability, so they refuse a `threshold`.
