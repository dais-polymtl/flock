...
-- Through OpenRouter, with an OpenRouter key
CREATE SECRET (
    TYPE TYPESAFE,
...
This creates a secret named `__default_typesafe`. `BASE_URL` is optional and defaults to TypeSafe itself.
...
### 3.5 Example for TypeSafe (Persistent):
...
Optionally add `BASE_URL 'https://openrouter.ai/api'` to use OpenRouter as the host.
