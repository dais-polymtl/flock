...
For structured output with Claude, use `output_format` (Claude 4.x) or rely on Flock's tool-use fallback (Claude 3.x). See the [Anthropic guide](/getting-started/anthropic) and examples on [`llm_complete`](/scalar-functions/llm-complete).
