...
The `typesafe` provider takes no model parameters: a TypeSafe request carries only a model, the rows and the questions, so `model_parameters` is refused for it. See [TypeSafe / Jev](/getting-started/typesafe).
