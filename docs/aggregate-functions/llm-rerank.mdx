...
SELECT llm_rerank(
  {'model_name': 'gpt-4o'},
  {
    'prompt': 'Rank by relevance to cutting-edge AI research',
    'context_columns': [
      {'data': document_title},
...
SELECT llm_rerank(
  {'model_name': 'gpt-4o'},
  {
    'prompt': 'Rank products by how appealing the product photo looks',
    'context_columns': [
      {'data': product_name},
...
SELECT llm_rerank(
  {'model_name': 'gpt-4o'},
  {
    'prompt': 'Rank clips by relevance to customer churn risk',
    'context_columns': [{
      'data': file_path,
      'type': 'audio',
...
- **Type**: JSON object.
- **Behavior**: Returns the input rows reordered by relevance to the prompt. The JSON mirrors the input column values. You can still pass provider `model_parameters` (including JSON schemas) to constrain how the ranking model responds — see [Model parameters](/model-parameters).
...
SELECT llm_rerank(
  ...
  {
    'prompt': 'Rank documents by relevance to cutting-edge AI research',
    'context_columns': [
      {'data': document_title},
      {'data': document_content}
    ]
