...
`llm_rerank` accepts the same provider `model_parameters` as other LLM functions. Use a JSON schema when you need the underlying model responses constrained (OpenAI `response_format`, Ollama `format`, Anthropic `output_format` / tool-use). See [Model parameters](/model-parameters).
