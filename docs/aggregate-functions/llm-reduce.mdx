...
Use `model_parameters` with a JSON schema so the reduce result is structured JSON (OpenAI `response_format`, Ollama `format`, or Anthropic `output_format` / tool-use). Load `JSON` to extract fields.
