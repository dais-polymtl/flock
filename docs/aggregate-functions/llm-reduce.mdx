...
SELECT
  ...
  llm_reduce(
    {'model_name': 'gpt-4o'},
    {
      'prompt': 'Summarize the key decisions from these audio clips.',
      'context_columns': [{
        'data': file_path,
        'type': 'audio',
...
**Free-form example**: aggregating product descriptions might return a single summary string such as *"A variety of products including running shoes, wireless headphones, and smart watches…"*.
...
SELECT
  product_id,
  analysis.overall_summary::VARCHAR AS overall_summary,
  analysis.key_themes AS key_themes
FROM (
  SELECT
    product_id,
    llm_reduce(
          ...
          "response_format": {
            ...
            "json_schema": {
              ...
              "schema": {
                ...
                "required": ["overall_summary", "key_themes"],
      ...
      {
        'prompt': 'Summarize these reviews into an overall summary and key themes.',
