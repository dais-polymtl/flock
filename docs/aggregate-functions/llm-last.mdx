...
Constrain the ranking model with a JSON schema via `model_parameters` (OpenAI `response_format`, Ollama `format`, Anthropic `output_format` / tool-use). The function still returns the selected row's columns as JSON.
...
SELECT llm_last(
  ...
  {
    'prompt': 'Pick the least relevant product for a runner',
    'context_columns': [
      {'data': product_name},
      {'data': product_description}
    ]
