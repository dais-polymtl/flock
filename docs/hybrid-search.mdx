...
Fusion functions operate on **numeric ranks or scores**, not on raw text, images, or audio. Build those scores however you like — including from [scalar](/scalar-functions/overview) or [aggregate](/aggregate-functions/overview) LLM pipelines — then fuse them with `fusion_*`.
