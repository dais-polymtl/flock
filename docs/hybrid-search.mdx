---
title: "Hybrid Search"
description: "Scalar fusion functions for combining retrieval scores in hybrid search pipelines."
---

## Overview

Flock provides a set of **scalar** fusion functions that allow you to combine multiple scoring systems into a single, unified
...
See also the [Scalar Functions overview](/scalar-functions/overview) for how fusion functions fit alongside `llm_complete` and related map functions.

Fusion functions operate on **numeric ranks or scores**, not on raw text, images, or audio. Build those scores however you like — including from multimodal LLM pipelines — then fuse them with `fusion_*`.
...
system. Described by [Cormack et al](https://doi.org/10.1145/1571941.1572114).
