...
Filter rows using tabular string columns in `context_columns`.
...
Use `type: 'image'` with a vision-capable model to filter on visual criteria.
...
WHERE llm_filter(
    {'model_name': 'gpt-4o'},
    {
      'prompt': 'Is this a high-quality professional product photo?',
      'context_columns': [
...
Filter on spoken content with `type: 'audio'` and `transcription_model` (OpenAI / Azure).

```sql
SELECT audio_id, customer_id, file_path
FROM VALUES
...
- **Parameter**: `prompt` or `prompt_name` with `context_columns`
  ...
  #### 2.2.4 Decision Threshold (TypeSafe models)

  A `typesafe` model answers with the probability that a row satisfies the prompt. `threshold` sets the probability at or above which the row counts as `true`, overriding the model's own. A `typesafe` model requires one, either here or on the model. It must be a constant between 0 and 1. Other providers ignore it.

  - **Example**:
    ```sql
    { 'prompt': 'The review complains about battery life.', 'context_columns': [{'data': review}], 'threshold': 0.9 }
    ```
