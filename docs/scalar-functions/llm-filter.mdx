...
#### 2.1.3 Decision Threshold (TypeSafe models)

- **Description**: A `typesafe` model answers with the probability that a row satisfies the prompt. `threshold` sets the probability at or above which the row counts as `true`. A `typesafe` model requires one, set here or in `CREATE MODEL`. It must be between 0 and 1. Other providers ignore it.
- **Example**:
  ```sql
