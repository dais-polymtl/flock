...
- **Parameter**: `prompt` or `prompt_name` with `context_columns`
  ...
  A `typesafe` model answers with the probability that a row satisfies the prompt. `threshold` sets the probability at or above which the row counts as `true`, overriding the model's own (default `0.5`). It must be a constant between 0 and 1, and other providers refuse it.
