...
Pass a JSON schema in `model_parameters` so the model returns schema-compliant JSON. Requires DuckDB's JSON extension to extract fields with dot notation (`LOAD JSON`).

**OpenAI** uses `response_format` with `json_schema`. **Ollama** uses `format`. **Anthropic / Claude** uses `output_format` or tool-use — see [Anthropic](/getting-started/anthropic) and [Model parameters](/model-parameters).
...
SELECT
  product_name,
  response.category::VARCHAR AS category,
  response.confidence::DOUBLE AS confidence
FROM (
  SELECT
    product_name,
    llm_complete(
  ...
  FROM UNNEST(['Wireless Bluetooth Headphones', 'Organic Cotton T-Shirt']) AS t(product_name)
);
```
