...
for `llm_complete`, `llm_filter`, `llm_embedding`, `llm_reduce`, `llm_rerank`, `llm_first`, `llm_last`, and `ai_classify` directly from
SQL.
...
- Tune prompts and batch sizes for cost/performance trade-offs (see [Performance and cost](/performance)).
