...
For a CLion-specific walkthrough (open the DuckDB CMake project, vcpkg, and run configs), see [`docs/development/clion.md`](https://github.com/dais-polymtl/flock/blob/main/docs/development/clion.md) in the repository. That guide is not part of this docs site.
...
- Scalar functions, `llm_first` and `llm_last` hand a provider each batch as a `StructuredCompletionRequest` before it is rendered. By default
  `Model` renders the prompt; a provider that builds its request from the rows themselves overrides
  `AcceptsStructuredTuples()` and `AddStructuredCompletionRequest()`, as the TypeSafe adapter does.
- What a provider can serve, and which settings apply to it, is checked when a query is bound: see
  `Model::RejectUnsupportedFunction` and `DescribeInapplicableModelArg`.

For examples, see the existing OpenAI, Azure, Ollama, Anthropic, and TypeSafe adapters.
