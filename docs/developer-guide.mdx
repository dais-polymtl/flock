...
- **Docs site**: Mintlify site under `docs/` (this documentation).
...
Flock CI produces DuckDB-WASM extension artifacts via `extension-ci-tools` (see `.github/workflows/MainDistributionPipeline.yml`). For browser usage and limitations, see [Browser and WASM](/wasm).
...
- `what-is-flock.mdx` for high-level positioning
- The relevant function or feature page (e.g. `image-support.mdx`, `audio-support.mdx`, `llm-metrics.mdx`)
- [`faq.mdx`](/faq) and [`static/llms.txt`](/static/llms.txt) so entry points stay current
