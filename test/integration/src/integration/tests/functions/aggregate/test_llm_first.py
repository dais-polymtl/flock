...
def test_llm_first_with_audio_transcription(integration_setup, transcription_model_config):
    ...
    query = (
        ...
        + """'},
            {
                'prompt': 'Which candidate has the best audio interview? Return the ID number only.',
                'context_columns': [
                    {
                        'data': audio_url,
                        'type': 'audio',
...
def test_llm_first_audio_ollama_error(integration_setup):
    ...
    create_model_query = "CREATE MODEL('test-ollama-first-audio', 'llama3.2', 'ollama');"
    run_cli(duckdb_cli_path, db_path, create_model_query)

    transcription_model_name = "test-ollama-first-transcription"
    create_transcription_model_query = (
        "CREATE MODEL('test-ollama-first-transcription', 'llama3.2', 'ollama');"
    )
    run_cli(duckdb_cli_path, db_path, create_transcription_model_query)
    ...
    query = """
        SELECT llm_first(
            {'model_name': 'test-ollama-first-audio'},
            {
                ...
                'context_columns': [
                    {
                        'data': audio_url,
                        'type': 'audio',
                        'transcription_model': 'test-ollama-first-transcription'
                    }
                ]
            }
        ) AS result
