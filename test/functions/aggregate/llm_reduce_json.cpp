...
protected:
    static constexpr const char* EXPECTED_JSON_RESPONSE = R"({"output": {"summary": "A comprehensive summary of running shoes, wireless headphones, and smart watches, featuring advanced technology and user-friendly designs for active lifestyles."}})";
    ...
    std::string GetFunctionName() const override {
        return "llm_reduce_json";
    }

    AggregateFunctionType GetFunctionType() const override {
        return AggregateFunctionType::REDUCE_JSON;
    }
...
TEST_F(LLMReduceJsonTest, LLMReduceJsonWithoutGroupBy) {
    ...
    const auto expected_json = GetExpectedJsonResponse()["output"].dump();
...
TEST_F(LLMReduceJsonTest, Operation_LargeInputSet_ProcessesCorrectly) {
    ...
    con.Query("CREATE TABLE test_large_json_dataset (id INTEGER, description VARCHAR);");

    std::string insert_query = "INSERT INTO test_large_json_dataset VALUES ";
...
TEST_F(LLMReduceJsonTest, Operation_ComplexJsonStructure_HandlesCorrectly) {
    ...
    complex_response["output"] = nlohmann::json::object();
    complex_response["output"]["metadata"] = {
            {"total_products", 3},
            {"analysis_timestamp", "2025-06-10T10:00:00Z"},
            {"version", "1.0"}};
    complex_response["output"]["insights"] = {
    ...
    complex_response["output"]["summary"] = "Comprehensive analysis of diverse product portfolio";
