...
std::vector<std::string> FusionCombANZ::Operation(duckdb::DataChunk& args) {
    ...
    for (int i = 0; i < num_different_scores; i++) {
        ...
        extracted_scores = Normalizer::normalize(extracted_scores, hit_counts, NormalizationMethod::Max);
