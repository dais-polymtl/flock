...
void ModelParser::ParseCreateModel(Tokenizer& tokenizer, std::unique_ptr<QueryStatement>& statement) {
    ...
    // The JSON argument is optional. If present, extract tuple_format, batch_size, max_async_calls, and model_parameters.
    if (token.type == TokenType::SYMBOL || token.value == ",") {
        token = tokenizer.NextToken();
        try {
            nlohmann::json input_args = nlohmann::json::parse(token.value);
            // Only allow tuple_format, batch_size, max_async_calls, model_parameters
            for (auto it = input_args.begin(); it != input_args.end(); ++it) {
                const std::string& key = it.key();
                if (key == "tuple_format" || key == "batch_size" || key == "max_async_calls" || key == "model_parameters") {
                    ...
                    if (key == "batch_size") {
                        ...
                        if (!param_val.is_number_integer()) {
                            throw std::runtime_error("Expected 'max_async_calls' to be an integer.");
                        }
                        model_args[key] = param_val.get<int>();
                    } else {
